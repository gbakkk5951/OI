...
_Main(){
	...
	for(i=1;i<=tmx;i++){
		...
		for(j=1;j<=pmx;j++){
			...
			if(sum.val-q[head].val>mxdec){
				tmp="use ("+tostring(q[head].r)+","+tostring(j)+"]";
			}
			mxdec=max(mxdec,sum.val-q[head].val);
		}
		if(dp[min(r+i+s,tmx+1)]>dp[i]+lost[min(r+i+s-1,tmx)]-lost[i-1]-mxdec){
			way[min(r+i+s,tmx+1)]=tmp;
		}
