...
void FFT(complex<double> *l,complex<double> *r,char iter){
	if(l==r){
		return;
	}
	int pn=r-l+1;
	...
	memcpy(l,temp,pn*sizeof(complex<double>));
	FFT(l,l+(r-l>>1),iter+1);
	FFT(l+(r-l>>1)+1,r,iter+1);
	complex<double>w=1;
	memcpy(temp,l,pn*sizeof(complex<double>));
	j=pn>>1;
	complex<double>t;
	for(i=0;i<j;i++){
		t=w*temp[i+j];
