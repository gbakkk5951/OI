...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		int n = 100000, l, r;
		randRange(1, n - 1, l, r);
		cout << n << endl << l << sp << r << endl;
		mchain(1, 2, n, cout);
//		int n = 1e5, l = 5e4, r = 5e4;
		
		/*mchain(1, 2, 3e4 + 1, cout);
		mflower(1, 3e4 + 2, 7e4 + 1, cout);
		mchain(1, 7e4 + 2, 1e5 - 1, cout);
		cout << (int)(1e5 - 1) << sp << (int)(1e5) << sp << (int)1e6 << endl;*/
		/*
		int i, use;
		mchain(1, 2, 5e4, cout);
		mflower(5e4, 5e4 + 1, 7e4, cout);
		mflower(1, 7e4 + 1, 1e5, cout);*/
//		mbtree(1, 9e4 + 1, 1e5, cout);
		/*
		for (i = 1, use = 1; n - use >= i; use += i++) {
			mchain(1, use + 1, use + i, cout);
		}
		if (use < n) {
			mchain(1, use + 1, n, cout);
		}
		*/
		EndFor1:
		cout.close();
...
void add(int a, int b, ostream &cout) { //要求编号较大的点是第一次连边 
	...
	if (rand() & 1) swap(a, b);
	cout << a << sp << b << sp << lrand(0, 1000000) << endl;
}
