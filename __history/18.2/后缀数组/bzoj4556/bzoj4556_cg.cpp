...
public:
	...
	Node *insert(int val, Node* pre, int l, int r) {
		Node* nd = new_node();
		if (l == r) {
			nd->sum = 1;
			return nd;
		...
		int mid = l + r >> 1;
		int pos = val > mid;
		nd->s[pos] = pos ? insert(val, pre->s[pos], mid + 1, r) 
					: insert(val, pre->s[pos], l, mid);
		nd->s[pos ^ 1] = pre->s[pos ^ 1];
		nd->sum = nd->s[0]->sum + nd->s[1]->sum;
		return nd;
	}
	int query(Node *lnd, Node* rnd, int l, int r, int nl, int nr) {
		if (l <= nl && nr <= r) {
			return rnd->sum - lnd->sum;
		}
		int mid = nl + nr >> 1;
		return (l <= mid ? query(lnd->s[0], rnd->s[0], l, r, nl, mid) : 0) 
			 + (r > mid ? query(lnd->s[1], rnd->s[1], l, r, mid + 1, nr) : 0);
	}
	int query(int lidx, int ridx, int l, int r) {
		return query(root[lidx], root[ridx], l, r, 0, mx);
	}
	void insert(int pos) {
		root[ridx++] = insert(pos, root[ridx - 1], 0, mx);
	}
