...
struct _TimeWatcherType_{
    ...
    char s[5];  
    int main()  
    {_TimeWatcherArray_[7]++;  
        freopen("KA0.in", "r", stdin);
        int t,i,j,k,ans,Max;  
        scanf("%d",&t);  
        while(t--)  
        {_TimeWatcherArray_[8]++;  
            Max=1;  
            scanf("%d",&n);  
            for(i=0;i<n;i++)  
            {_TimeWatcherArray_[9]++;  
                scanf("%s",s);  
                r[i]=s[0]-'a'+1;  
            }  
            r[i]=0;  
            ...
            RMQ();  
            for(i=1;i<=n;i++)  
            {_TimeWatcherArray_[10]++;  
                for(j=0;j+i<n;j+=i)  
                {_TimeWatcherArray_[11]++;  
                    ans=calprefix(j,j+i);  
                    k=j-(i-ans%i);  
                    ans=ans/i+1;  
