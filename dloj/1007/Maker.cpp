...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		if (I == 0) {
			...
			static int src[1005];
			for (int i = 1; i <= sq; i++) {
				src[i] = lrand(0, (1<< 15) - 1);
			}
			
			for (int i = 0; i * 2 * sq <= n; i++) {
				for (int j = 1; i + j <= n && j <= 2 * sq; j++) {
					if (j <= sq) {
						cout << (1 << 15 | src[j]) << sp;
					} else {
						cout << (src[2 * sq + 1 - j]) << sp;
					}
				}
			}
			cout << endl;
			...
			for (int i = 1; i <= m; i++) {
				...
				if (i & 1) {
					...
					cout << 1 << sp << (1 << 15) << sp << ((j - 1) * 4 * sq + sq) 
					<< sp << ((j - 1) * 4 * sq + 3 * sq) << endl;
					j++;
				} else {
					randRange(1, n, l, r);
					cout << 2 << sp << l << sp << r << endl;
				}
			}
			...
			for (int i = 1; i <= n; i++) {
				cout << bit_rand(0, 5e4) << sp;
			}
			cout << endl;
			for (int i = 1; i <= m; i++) {
				int l, r;
				bit_randRange(1, n, l, r);
				if (rand() & 1) {
					cout << 1 << sp << bit_rand(0, 5e4) << sp; 
				} else {
					cout << 2 << sp;
				}
				cout << l << sp << r << endl;
			...
			for (int i = 1; i <= n; i++) {
				cout << bit_rand(0, 5e3) << sp;
			}
			cout << endl;
			for (int i = 1; i <= m; i++) {
				int l, r;
				if (rand() % 10 < 9) {
					l = lrand(1, 5e2); r = lrand(n - 5e2, n);
				} else {
					randRange(1, n, l, r);
				}
				if (rand() % 3 == 1) {
