...
struct cp{
	double a,b;
	cp operator +(const cp &o)const{/* Counter: 31457280 */return (cp){a+o.a,b+o.b};}
	cp operator -(const cp &o)const{/* Counter: 31457280 */return (cp){a-o.a,b-o.b};}
	cp operator *(const cp &o)const{/* Counter: 33030141 */return (cp){a*o.a-b*o.b,b*o.a+a*o.b};}
	cp operator *(const double &o)const{/* Counter: 0 */return (cp){a*o,b*o};}
	cp operator !()const{/* Counter: 0 */return (cp){a,-b};}
}x[1050000],y[1050000],z[1050000],w[1050000];
void fft(cp x[],unsigned k,unsigned v){/* Counter: 3 */
	for(unsigned i=0,j=0;i<k;i++){/* Counter: 3145728 */
		if(i>j)swap(x[i],x[j]);
		for(unsigned l=k>>1;(j^=l)<l;l>>=1);
	}
	w[0]=(cp){1,0};
	for(unsigned i=2;i<=k;i<<=1){/* Counter: 60 */
		cp g=(cp){cos(2*pi/i),(v?-1:1)*sin(2*pi/i)};
		for(int j=(i>>1);j>=0;j-=2)w[j]=w[j>>1];
		for(unsigned j=1;j<i>>1;j+=2)w[j]=w[j-1]*g;
		for(unsigned j=0;j<k;j+=i){/* Counter: 3145725 */
			cp *a=x+j,*b=a+(i>>1);
//#pragma GCC ivdep
			for(unsigned l=0;l<i>>1;l++){/* Counter: 31457280 */
				cp o=b[l]*w[l];
				b[l]=a[l]-o;
				a[l]=a[l]+o;
			}
