...
void make(){
	...
	for(I=beg;I<end;I++){
		...
		int n = 150000, A = 1e9, Qn = 200000;
		cout << n << sp << Qn << sp << A << endl;
		for (int i = 1; i <= n; i++) {
			cout << rand() % A << sp;
		}
		mworm(1, 2, n, cout);
		...
		add(1, 2, cout);
		mbtree(2, 3, n / 3, cout);
		mchain(1, n / 3 + 1, n / 3 * 2, cout);
		add(1, n, cout);
		mworm(n, n / 3 * 2 + 1, n - 1, cout);
		*//*
		for (int i = 2; i <= n; i++) {
			//cout << i << sp << lrand(1, i - 1) << sp << lrand(1, 1000) << endl;
			//cout << i << sp << (i >> 1) << sp << lrand(1, 1e3) << endl;
		}
		*/
		for (int Q = 1; Q <= Qn; Q++) {
			cout << lrand(1, n) << sp << lrand(0, A - 1) << sp << lrand(0, A - 1) << endl;
		}
...
void add(int a, int b, ostream &cout) { //要求编号较大的点是第一次连边 
	if (rand() & 1) swap(a, b);
	cout << a << sp << b << sp << lrand(1, 1000) << endl;
}
