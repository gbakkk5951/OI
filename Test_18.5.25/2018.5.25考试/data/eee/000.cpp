...
long long sum[gug]={};long long minnn[gug]={},maxx[gug]={},v[gug]={};
bool vis2[gug]={};
long long tee;int codeforces=10000000;
ff get0(long long a){
	a=-a;long long b=1;
	while(a<0)a=a+le[b++];
	return (ff){b,a};
...
void insert3(long long a,long long b,long long c,long long d,long long e,long long f){
	...
	{
		vis2[e]=1;sum[e]=f;
		sum[e]*=(b-a+1);minnn[e]=maxx[e]=v[e]=f;
		return;
	}
	if(vis2[e])
	{
		...
		sum[e*2]=sum[e];sum[e*2]/=(d-c+1);sum[e*2+1]=sum[e*2];
		sum[e*2]*=d-c+2>>1,sum[e*2+1]*=d-c+1>>1;
	}
	else if(v[e])
	{
		long long tee=v[e];v[e*2]+=v[e],v[e*2+1]+=v[e];
		minnn[e*2]+=v[e],minnn[e*2+1]+=v[e];maxx[e*2]+=v[e],maxx[e*2+1]+=v[e];
		sum[e*2]+=tee*(d-c+2>>1),sum[e*2+1]+=tee*(d-c+1>>1);v[e]=0;
	}
	int zhong=c+d>>1;
	...
	sum[e]=sum[e*2]+sum[e*2+1];sum[e]+=(d-c+1)*v[e];
...
long long findsum(int a,int b,int c,int d,int e){
	if(a==c&&b==d)
		return sum[e];
	if(vis2[e])
	{
	...
	return findsum(a,zhong,c,zhong,e*2)+findsum(zhong+1,b,zhong+1,d,e*2+1);
}
int main()
{
	freopen("1","r",stdin);
	...
	for(long long ii=0;ii<qqq;ii++)
	{
		...
		case 2:
			...
			while(true)
			{
				...
				if(err)insert2(x,y,1,n,1,1),insert4(x,y,1,n,1,1),err=0;
				else break;
			}
			break;
		case 3:
		...
		case 6:
			...
			tee/=(y-x+1);
			insert1(x,y,1,n,1,get0(tee));
			insert3(x,y,1,n,1,tee);
			if(minn[1]==0)err=1;
			while(true)
			{
				while(true){
